  ...
  try {
    ...
    if (req.body.content) {
      if (!req.body.content || req.body.content.trim().length === 0) {
        return next(errorHandler(400, "Review content can not be empty!"));
      }
      if (!req.body.content || req.body.content.trim().length === 300) {
        return next(errorHandler(400, "Review content can not be more than 300 charactors!"));
      }
    }
    if (req.body.rating) {
      if (!req.body.rating) {
        return next(errorHandler(400, "Rating can not be empty!"));
      }
    }
...
export const getModarateRating = async (req, res, next) => {
  try {
    ...
    const totalratingValue =
      Fivestar * ratingValue[5] +
      Fourstar * ratingValue[4] +
      Threestar * ratingValue[3] +
      Twostar * ratingValue[2] +
      Onestar * ratingValue[1];
    ...
    const moderateRating =
      totalRatings !== 0 ? (totalratingValue / totalRatings).toFixed(1) : 0;
...
export const deleteReview = async (req, res, next) => {
  try {
    ...
    if (review.userrefId !== req.user.id && !req.user.isAdmin) {
      return next(
        errorHandler(403, "Your are not allow to delete this review")
      );
    }
